...
import { fetchTurbosBotState, fetchThalaBotState } from '../services/botState'

const REFRESH_INTERVAL = 60_000

const INITIAL_CAPITAL = 105 // $55 DEEP/USDC + $50 APT/USDC
...
export function usePoolData() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    const [sui, aptos, turbosState, thalaState] = await Promise.all([
      fetchSuiPoolData(),
      fetchAptosPoolData(),
      fetchTurbosBotState(),
      fetchThalaBotState(),
  ...
  const totalPositionUsd = (suiPool?.positionValueUsd || 0) + (aptosPool?.positionValueUsd || 0)
  const totalIdleUsd = (suiPool?.walletBalance?.totalIdleUsd || 0) + (aptosPool?.walletBalance?.totalIdleUsd || 0)
  const totalValueUsd = totalPositionUsd + totalIdleUsd
  const totalFeesUsd = (suiPool?.pendingFeesUsd || 0) + (aptosPool?.pendingFeesUsd || 0)
  const totalRewardsUsd = (suiPool?.pendingRewardsUsd || 0) + (aptosPool?.pendingRewardsUsd || 0)
